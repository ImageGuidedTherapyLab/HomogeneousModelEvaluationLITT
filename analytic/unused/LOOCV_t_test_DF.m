...
%     mu_a_iter = (-3*mu_s_p + sqrt( 9*mu_s_p^2 + 12 * mu_eff_iter^2))/6; % Used quadratic equation to solve for mu_a in terms of g, mu_s, and mu_eff
    ...
    input_file = cell (31,1);
    input_file {1,1} = '20 variables';
    input_file {2,1} = %%%
    input_file {3,1} = strcat ( num2str(mu_eff_iter),' mu_eff_healthy');
    input_file {4,1} = %%%%
    input_file {5,1} = %%%
    input_file {6,1} = strcat ( num2str(alpha_iter), ' alpha_healthy');
    input_file {7,1} = %%%
    input_file {8,1} = '1.045000000000000e+03 rho_healthy';
    input_file {9,1} = '9.000000000000000e-01 anfact_healthy';
    input_file{10,1} = '6.000000000000000e+00 w_0_healthy';
    input_file{11,1} = '3.700000000000000e+01 body_temp';
    input_file{12,1} = %%%;
    input_file{13,1} = %%%;
    input_file{14,1} = '2.100000000000000e+01 probe_init';
    input_file{15,1} = '8.000000000000000e+03 mu_s_healthy';
    input_file{16,1} = '3.640000000000000e+03 c_p_healthy';
    input_file{17,1} = '0.000000000000000e+00 robin_coeff';
    input_file{18,1} = '3.840000000000000e+03 c_blood_healthy';
    input_file{19,1} = %%%
    input_file{20,1} = %%%
    input_file{21,1} = %%%
    input_file{22,1} = '2 functions';
    input_file{23,1} = '1 ASV_1:obj_fn_1';
    input_file{24,1} = '1 ASV_2:obj_fn_2';
    input_file{25,1} = '5 derivative_variables';
    input_file{26,1} = '1 DVV_1:x_displace';
    input_file{27,1} = '2 DVV_2:mu_eff_healthy';
    input_file{28,1} = '3 DVV_3:y_displace';
    input_file{29,1} = '4 DVV_4:z_displace';
    input_file{30,1} = '5 DVV_5:alpha_healthy';
    input_file{31,1} = '0 analysis_components';
