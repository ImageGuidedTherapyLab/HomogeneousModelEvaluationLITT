...
function DAKOTA_in_writer_naive ( path, mu_opt, alpha_opt, best_iter, opttype );
...
copy_command = strcat('cp ./', path_base, 'optpp_pds.', opttype, '.in.', num2str(best_iter), ' ./', path_base, 'optpp_pds.naive.in.1');
disp(copy_command);
...
mu_sed_command = strcat( 'sed -i "0,/',orig_mu,'/s/',orig_mu,'/',  new_mu, '/" ./',path_base,'optpp_pds.naive.in.1');
disp(mu_sed_command );
...
alpha_sed_command = strcat( 'sed -i "0,/', orig_alpha, '/s/', orig_alpha, '/', new_alpha, '/" ./', path_base,'optpp_pds.naive.in.1'  );
