        ...
        if length(total_toss{ii,jj}) < length(opt.paths) -1
            if jj >1
                if length(total_toss{ii,jj}) == length(total_toss{ii,jj-1})
                    ...
                    kk = kk+1;
                    [ LOOCV.dice.hh{ii,jj}, LOOCV.dice.values{ii,jj}] = naive_t_test_DiceTemp( LOOCV.paths.paths{ii,jj}, opt.mu_eff.values{ii,jj}, alpha{ii,jj}, best_iter{ii,jj}, opttype);
                    LOOCV.run1(ii,jj) = 2;
                    LOOCV.run2(ii,jj) = 1;
                ...
                mu_eff = opt.mu_eff.values{ii,jj};
                mu_eff(:) = 180;
