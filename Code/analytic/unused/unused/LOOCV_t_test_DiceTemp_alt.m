...
function [ hh, dice_values ] = LOOCV_t_test_DiceTemp_alt ( Study_paths, mu_eff_DpassT, opt_type ); % The alt case averages a cohort and applies it to rest.

% Make the LOOCV iteration system
alt_index = 1:25;
alt_index( mu_eff_DpassT(:,2)) = [];
n_patients = size( alt_index,2); % This is the number of patients
% n_patients = 1;
...
for ii = 1:n_patients
    ...
    % thermal code to run
    param_file = strcat( 'workdir/', Study_paths{alt_index(ii),1}, '/', Study_paths{alt_index(ii),2}, '/opt/', opt_type, '.in.1');
    ...
    mu_eff_iter = mu_eff_DpassT(:,3); % Make a copy of both the mu_eff values and the paths
